...
cp "$BASEDIR/chroot_steps_part-1.sh" "$BASEDIR/chroot_steps_part-2.sh" chroot
...
chroot chroot/ /bin/bash -c "IS_INTEGRATION_TEST=$IS_INTEGRATION_TEST ARCH=$ARCH CODENAME=$CODENAME /chroot_steps_part-1.sh"
if [[ $? -ne 0 ]]; then
...
# Enter chroot again
cd "$BUILD_DIRECTORY"
chroot chroot/ /bin/bash /chroot_steps_part-2.sh
if [[ $? -ne 0 ]]; then
...
rm chroot/chroot_steps_part-1.sh chroot/chroot_steps_part-2.sh
